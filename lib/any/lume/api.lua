...
--- of their iteratee function argument. The functions that provide this behavior
...
--- is specified the accumulator is initialized to this, otherwise the first value
...
--- Mimics the behavior of Lua's `string.sub`, but operates on an array rather
...
--- specified an array of the first `n` elements is returned.
--- ```lua
...
--- specified an array of the last `n` elements is returned.
--- ```lua
...
--- Returns a wrapper function to `fn` which takes the supplied arguments. The wrapper
--- function will call `fn` on the first call and do nothing on any subsequent calls.
--- ```lua
